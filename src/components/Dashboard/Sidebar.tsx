			...
			{routes.map((route, index) => {
				return route.name === "Connectivity" ? (
					<div className="w-full h-full">
						<div role="button" onClick={() => setShowConnectArray(p => !p)} className="trigger py-2  w-full flex items-center justify-between pr-3">
							<div className="gap-6 flex items-center ">
								<Image src={route.icon} width={24} height={24} alt="icon" />
								<p>{route.name}</p>
							</div>

							<Icons.ArrowIcon className={cn("transition-all", showConnectAray ? "rotate-90" : "rotate-0")} />
						</div>
						...
						{showConnectAray && (
							<div className="list w-full px-2 space-y-1">
								{connectivitySubList.map((item, index) => (
									...
									>
										<Image src={item.icon} width={24} height={24} alt="icon" className="invisible" />

										<span
											className={cn(
												"opacity-70",
												pathname === item.link || pathname.includes(item.link) ? "text-button-primary" : ""
											)}
										>
											{item.name}
										</span>
					...
					<Link href={route.link} key={index} className={"flex items-center gap-6"} onClick={() => setOpen?.(false)}>
						<Image src={route.icon} width={24} height={24} alt="icon" />
						<span
							className={cn(
								"opacity-70",
								pathname === route.link || pathname.includes(route.link) ? "text-button-primary" : ""
							)}
						>
							{route.name}
						</span>
